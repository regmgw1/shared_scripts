...
while (my $line = <IN>)
{
	...
	#my @count = `intersectBed -a tmp$time.tmp -b $path2feature/cpg_island_subsets/$line"."gff -wa -wb -f $intersect_thresh`;
	...
	#my @count2 = `intersectBed -a $path2feature/cpg_island_subsets/$line"."gff -b tmp$time.tmp -wa -wb -f $intersect_thresh`;
