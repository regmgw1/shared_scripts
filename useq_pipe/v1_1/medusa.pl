...
my $path2useq = $paths->{path2filtered};
...
my $bypass = $thresholds->{bypass_variance_outlier_filter};
...
if ($part0)
{
	...
	foreach my $child ( 0 .. $#samples )
	{
		...
		print "started alignment process for  $sample\n";
	        exec("$path2scripts/medusa_pipeline/medusa_pipe_pt0.sh $sample $path2reads $path2genome_dir $read1 $read2 $bwa_genome $fasta_genome $path2alignment $max_insert $bwa_threads") or die "couldn't exec part 0 $sample";
...
if ($part3)
{
	...
	foreach my $child ( 0 .. $#samples )
	{
		...
		exec("$path2scripts/medusa_pipeline/medusa_pipe_pt3.sh $sample $path2useq $ref_name $path2useqJava") or die "couldn't exec part 3 $sample";
...
if ($part4)
{
	...
	system("$path2scripts/medusa_pipeline/medusa_pipe_pt4.sh $treat_cohort $control_cohort $path2dmr $path2useq $min_read_depth $min_fdr $bypass $path2useqJava");
...
if ($part5)
{
	...
	system("$path2scripts/medusa_pipeline/medusa_pipe_pt5.sh $path2dmr $path2genome_dir/$fasta_genome $path2features_dir/genes/genes.gff $upstream $downstream $path2features_dir $path2features_list $path2repeats_list $intersect_threshold $path2scripts/perl_scripts $path2annotation_output $path2useq $path2cells_list $species $path2genome/$chrom_list");
