...
if ($part5)
{
	...
	system("$path2scripts/medusa_pipeline/medusa_pipe_pt5.sh $path2dmr $path2genome_dir/$fasta_genome $path2features_dir/genes/genes.gff $upstream $downstream $path2features_dir $path2features_list $path2repeats_list $intersect_threshold $path2scripts $path2annotation_output $path2useq $path2cells_list $species $path2genome/$chrom_list");
