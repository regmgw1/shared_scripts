...
=head2 Description
Use bedTools 'intersectBed' to determine how many reads are found under peaks.
=head2 Usage
...
foreach my $peaksub (@peak_subs)
{
	...
	foreach my $sample (sort @samples)
	{
		...
		foreach my $overlap (@overlaps)
		{
			...
			my $tmp_coord = $elems[0]."_".$elems[3];
			$coord{$tmp_coord} = $elems[4];
	...
	foreach my $chr_out (sort keys %peak)
	{
		...
		{
			my $tmp_start = $chr_out."_".$begin_out;
			print OUT "$chr_out\t$begin_out\t$coord{$tmp_start}";
